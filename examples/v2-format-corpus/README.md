# DocForge Format Corpus

这是一个可离线编译的 DocForge v1 完整格式语料。它把同一组 Markdown
语义对象贯穿到 `ForgeDocument`、`RenderPlan`、Review 和 DOCX/OpenXML，
用于验证内容与格式分离，而不是验证某一个孤立的 `.docx` 文件。

## 文件

- `docforge.yaml`：严格的 `docforge.project.v1` 项目入口、模板选择和布局覆盖。
- `document.md`：富文本、章节、列表、图表、公式、代码、引用、脚注和语义区语料。
...
```bash
.venv/bin/docforge inspect examples/v2-format-corpus
.venv/bin/docforge validate examples/v2-format-corpus --json
.venv/bin/docforge review examples/v2-format-corpus --output-dir /tmp/docforge-review
.venv/bin/docforge build examples/v2-format-corpus -o /tmp/v2-format-corpus.docx
...
- 三个 H1 语义区域、H2/H3 标题、TOC、cover、manifest metadata、main section、
