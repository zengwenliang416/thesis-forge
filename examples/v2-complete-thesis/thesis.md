...
本文构建一个面向学术论文的本地优先文档编译流程，将可读的 Markdown
源文件转换为经过模板约束、语义校验和 OpenXML 结构检查的可编辑 Word 文档。
系统不把 Markdown 直接写入 Word，而是先生成带有稳定身份和源代码位置的类型化
文档模型，再生成 RenderPlan，最后由 DOCX renderer 输出 Word。
...
# 第1章 绪论 {#chap:introduction}

## 研究背景 {#sec:background}

论文写作同时包含内容组织、资源管理、引用解析、公式排版和学校格式约束。
传统方式通常要求作者在 Word 中手工维护目录、题注和交叉引用，导致源内容与
最终排版之间出现难以追踪的差异。已有研究认为，结构化文档管线有助于保持
内容身份和发布结果之间的一致性 [@smith2025]。

本项目把项目目录作为唯一入口。构建流程依次经过解析、校验、编译、渲染、
最终化、OpenXML postflight 和预览阶段。完整流程见[图](#fig:architecture)，
损失函数示例见[式](#eq:loss)，实验结果见[表](#tbl:results)。
...
![论文编译流水线](assets/architecture.png){#fig:architecture}
...
设论文编译结果的质量由结构完整性、内容可读性和模板一致性共同决定，则可以
使用如下目标函数描述：
...
其中，$S$ 表示结构完整性，$R$ 表示 Review 可读性，$T$ 表示模板一致性。
在实验中，损失函数可以写为：
...
当编译器发现[式](#eq:quality)或[式](#eq:loss)的 ID 重复、引用目标不存在
或公式无法转换为 OMML 时，构建应返回结构化诊断，而不是生成伪公式。

## 数据与实验表格 {#sec:experiment}
...
```python {#lst:training title="训练循环示例"}
...
代码中的 `{#literal}`、`[@literal]` 和 `@fig:literal` 都必须保持为字面量，
不能被当作论文 ID 或引用。
...
# 第3章 验证与失败恢复 {#chap:verification}

## 引用、脚注和源代码定位 {#sec:references}

每个引用都必须能够在 `references.bib` 中找到对应条目。本文引用
[@smith2025, p. 12] 说明了带 locator 的引用形式。交叉引用使用可读标签，
例如[回到研究背景](#sec:background)，而不是直接暴露内部 NodeId。

这里有一个说明性脚注[^recovery]。脚注正文会出现在 Review 的脚注区域，
DOCX 中则使用 `word/footnotes.xml` 和原生脚注引用。

[^recovery]: 失败构建不会覆盖上一次成功 DOCX；BuildReport 会记录失败阶段、
主诊断和可继续使用的旧输出。

## 构建报告 {#sec:build-report}
...
如果用户修改了源码导致构建失败，界面保留上一次成功预览，并明确显示
“预览已过期”，不能把旧结果伪装成当前结果。

# 第4章 讨论 {#chap:discussion}

## 可复现性 {#sec:reproducibility}

完整项目包可以被复制到另一台机器，在不依赖 AI 服务和网络的情况下重新执行
inspect、validate、review 和 build。模板、图片、BibTeX 和源代码均通过项目
相对路径解析，构建报告则保存本次运行的结构化证据。
...
本文示例说明，Markdown 转 Word 的完整交付物应当是一个可验证的项目，而不只是
一个孤立的二进制文件。通过 manifest、Typed IR、RenderPlan、Review、DOCX
postflight 和 BuildReport，论文内容、格式策略和最终输出之间建立了可追踪关系。
