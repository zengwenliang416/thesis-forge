...
基础编译必须在无网络、无 API Key、无数据库和无 AI 服务时运行。当前交付以 Python CLI
为唯一产品入口，桌面 UI 与 AI 均为后置扩展。

## Application Topology

- Frontend runtime: V1 核心没有前端运行时；未来桌面 UI 使用可选 PySide6 层。
- Backend runtime: 单个 Python 3.11+ 本地进程，不运行常驻 Web 服务。
- API gateway or edge layer: 无。
- Background workers: 无；所有编译步骤在当前 CLI 进程内同步执行。
- External services: 核心链路无外部服务。CSL 数据或学校模板作为本地文件加载。
- Local development entrypoints: `thesisforge inspect`、`thesisforge validate`、`thesisforge build`。
- Production deployment shape: Python package/CLI；输入、模板、图片、BibTeX 与输出 DOCX 均在本地文件系统。
...
- Responsibility: CLI 编排核心用例；未来 UI 调用同一 application service；AI 仅提供可选辅助。
- Public contract: `thesisforge inspect|validate|build`。
- Owned data: 命令参数和用户可见输出，不拥有 domain truth。
- Dependencies: Core application APIs。
- Forbidden dependencies: UI 或 AI 反向进入 Parser/Domain/Compiler；AI 成为 build 必需依赖。
- Extension points: 后续 PySide6 adapter 与 `src/thesis_forge/ai/` providers。

## Frontend Architecture

- Routing: 当前无页面路由。
- Rendering mode: 当前无 UI rendering；未来 PySide6 desktop adapter 不改变核心编译链路。
- State management: 当前 CLI 调用内的短生命周期状态。
- Form handling: 当前无表单。
- Data fetching: 仅本地文件读取。
- Error handling: Domain 返回结构化错误，CLI 负责展示与 exit code。
- Design system source: `openspec/specs/ui-design/design.md`，仅在 UI 里程碑启用。

## Backend Architecture

- API style: 无 HTTP/RPC；使用进程内 Python function contracts。
- Request validation: CLI 路径/选项校验、Parser、Template Model 与 Validator 分层负责。
...
- User roles: 单机当前用户。
- Permission checks: 依赖操作系统文件权限；CLI 不提升权限。
- Data isolation: 不上传论文内容；核心链路不联网。
...
- Third-party APIs: 核心无；未来 AI 仅在 `src/thesis_forge/ai/`。
- Webhooks: 无。
- Queues: 无。
- Email/SMS/push: 无。
- Payments: 无。
- Analytics: 核心无远程 analytics。
- Local libraries: PyYAML、Pydantic、python-docx、lxml、Typer、Rich；citeproc-py 与 PySide6 为可选依赖。
...
- Don't introduce a database, HTTP API, account system, or AI dependency for V1 core.
