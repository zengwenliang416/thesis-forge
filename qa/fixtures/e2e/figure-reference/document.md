...
手工排版在章节调整后容易产生图表编号与正文引用不一致的问题。模板驱动编译把编号计算集中在编译阶段，正文中的引用写作稳定 ID，最终编号由编译器统一解析。

## 研究内容 {#sec:scope}

本文围绕以下内容展开：

1. 定义带稳定 ID 的图、表、公式语义对象；
2. 编译阶段生成 SEQ 编号字段与书签；
3. 正文交叉引用生成指向书签的 REF 字段。

系统总体管线如[图](#fig:pipeline)所示。
...
实验统计结构校验与字段断言的通过情况，结果面板见[图](#fig:dashboard)，核心指标汇总见[表](#tbl:metrics)。
...
综合评分定义见[式](#eq:score)，其中权重由模板实验标定。
...
图[图](#fig:pipeline)、图[图](#fig:dashboard)、表[表](#tbl:metrics)与式[式](#eq:score)均可在 Word 客户端中继续编辑，其编号随章节增删自动刷新。
