...
function installedCacheRoot() {
  ...
  if (path.basename(pluginDir) === currentPluginName) {
    const marketplaceRoot = path.dirname(pluginDir);
    const cacheRoot = path.dirname(marketplaceRoot);
    if (path.basename(cacheRoot) === 'cache') return marketplaceRoot;
  }
  const codexHome = process.env.CODEX_HOME
    ? path.resolve(process.env.CODEX_HOME)
    : path.join(require('os').homedir(), '.codex');
  ...
  return fs.existsSync(marketplaceRoot) ? marketplaceRoot : null;
}
