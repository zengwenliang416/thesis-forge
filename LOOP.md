...
- [REG-015C-ATOM] Complete the manifest-backed browser project-entry migration
  ...
  - Files: `src/thesis_forge/adapters/runtime.py`, `src/thesis_forge/adapters/http.py`, `tests/test_adapters.py`, `frontend/src/transport/WorkbenchTransport.ts`, `frontend/src/transport/web.ts`, `frontend/src/transport/WorkbenchTransport.project.test.ts`, `frontend/src/components/ProductBar.tsx`, `frontend/src/components/ProductBar.project.test.tsx`, `frontend/src/components/WorkbenchShell.tsx`, `frontend/src/components/WorkbenchShell.project.test.tsx`, `frontend/src/components/WorkbenchApp.tsx`, `frontend/src/components/WorkbenchApp.project.test.tsx`, `LOOP.md`, `lint-loop.sh`
  - Behavior: Web project opening accepts exactly the manifest and Markdown source snapshots, derives the authoritative project identity from `thesisforge.yaml` on the server, returns one opaque workspace-backed project, and carries one canonical `File[]` selection through ProductBar, WorkbenchShell and WorkbenchApp into typed `openProject`.
  - Verify: `.venv/bin/python -m pytest tests/test_adapters.py -k "web_runtime or http_workspace_creation or manifest_project_identity" && pnpm --dir frontend exec vitest run src/transport/WorkbenchTransport.project.test.ts src/components/ProductBar.project.test.tsx src/components/WorkbenchShell.project.test.tsx src/components/WorkbenchApp.project.test.tsx && pnpm --dir frontend typecheck`
  - Acceptance: the request contains exactly `{manifest, source}`; the server validates the manifest/source binding and returns strict project identity; browser selection has one `File[]` callback with `multiple`; WorkbenchApp rejects incomplete or standalone selections without `openSource` fallback or frontend YAML parsing; Tauri project opening and all project operation identity payloads remain intact.
  - Verification-surface change: `authorized`; this is the explicit user-approved cross-layer exception to the three-file bound required to avoid a partial public-entry migration.
  - Attempts: 1
  - Attempt 1 (2026-08-24): Independent Checker PASS; exact Verify passed backend `9/9` and frontend `72/72`, frontend typecheck, frontend lint, target Ruff, `git diff --check`, and LOOP-LINT passed; no finding. Current-source audit confirmed exact `{manifest, source}` creation, manifest/source binding and manifest-derived identity, retained ProductBar `onChooseSource` routing, single `File[]` browser selection, no WorkbenchApp `openSource` fallback or YAML parsing, unchanged Tauri project picker and operation identity payloads, and split client-project/duplicate-metadata negative assertions; unrelated dirty paths were preserved and unstaged, no push.

- [REG-015B] Carry the manifest-backed project snapshot through Web transport
  - Parent: ordered child 2/4 of `REG-015`; depends on `REG-015A`; the original REG-015 Behavior and Acceptance remain unchanged across the ordered children.
  ...
  - Behavior: Web `openProject` serializes and strictly validates one typed manifest-backed project identity and declared source snapshot contract instead of routing project selection through `openSource`.
  - Verify: `pnpm --dir frontend test -- WorkbenchTransport.project.test.ts`
  - Acceptance: Web and Tauri project responses share the same strict identity/source contract; malformed, projectless or uploaded-file-only project responses fail explicitly; the request/response carries no duplicate metadata source or silent fallback.
  - Verification-surface change: authorized; extends the focused Web project transport regression.
  - Attempts: 1
  - Attempt 1 expected/observed evidence (2026-08-24): Expected: `OpenProjectInput` accepts exactly one project identity, `thesisforge.yaml` manifest snapshot and Markdown source snapshot; Web sends exactly `{project, manifest, source}` once to the workspace endpoint without legacy fields or `openSource` fallback; Web/Tauri project responses use one strict identity/source parser and reject malformed, projectless, duplicate-metadata, unknown-source and `web-upload` project responses. Observed: `readOpenProjectInput` and `readProjectFileSnapshot` enforce exact keys, types and file roles; Web issued one workspace POST with only the three required top-level keys; `readWebProjectResponse` and shared `readOpenedProject` rejected all requested malformed response variants while accepting only `desktop`/`web-workspace`; source-only Web `openSource` retained `web-upload`.
  - Attempt 1 verification (2026-08-24): exact `pnpm --dir frontend exec vitest run src/transport/WorkbenchTransport.project.test.ts` passed `52/52`; `pnpm --dir frontend test -- WorkbenchTransport.project.test.ts` passed `19` files/`231/231`; frontend `typecheck`, `lint`, `git diff --check` and `./lint-loop.sh` passed (`open=3 done=190 blocked=0` before this move); CodeGraph sync/explore confirmed the current Web → shared parser and Tauri → shared parser call paths; independent black-box probe passed `14` input mutants, `13` response mutants, one workspace POST, HTTP errors, Tauri/Web parity, project `web-upload` rejection and source-only `web-upload` retention; no `dto.ts` or out-of-scope product files changed.

- [REG-015A] Add manifest-backed Web workspace creation and persistence
  - Parent: ordered child 1/4 of `REG-015`; the original REG-015 Behavior and Acceptance remain unchanged across the ordered children.
  - Files: `src/thesis_forge/adapters/runtime.py`, `src/thesis_forge/adapters/http.py`, `tests/test_adapters.py`
  - Behavior: Web project opening creates one isolated workspace containing the uploaded `thesisforge.yaml` and its declared readable source, then returns the typed project/source snapshot used by subsequent dispatch operations.
  - Verify: `.venv/bin/python -m pytest tests/test_adapters.py -k "web_runtime or http_workspace_creation"`
  - Acceptance: manifest-backed workspace creation is strict and path-safe; metadata, resources and template selection remain manifest-owned; the returned workspace handle is opaque; bare Markdown is not accepted as a project entry; no fallback or compatibility branch is introduced.
  - Verification-surface change: authorized; extends the Web workspace adapter regression.
  - Attempts: 2
  - Inherited Attempt 1 (2026-08-24): The unsplit REG-015 exact Verify failed after the candidate source was changed to front-matter-free canonical Markdown. Upload and save returned successfully, but refresh remained `构建已禁用` with `missing-template` and required metadata diagnostics for `author.name` and `thesis.title`; CodeGraph and source inspection showed that Web `openSource` creates only a bare `web-workspace`, `POST /api/v1/workspaces` accepts only `fileName`/`text`, and V2 metadata/template values exist only in `thesisforge.yaml`; the candidate acceptance file was restored.
  - Attempt 1 expected/observed evidence (2026-08-24): Expected: project identity responses must not expose the runtime root, project opening and dispatch must reject project-only or legacy `web-upload` inputs, and the old bare `create_workspace(fileName, text)` path must remain unavailable to project creation. Observed: the earlier Web path leaked runtime `project.root`/`project.manifestPath` values and left project-only/`web-upload` fallback plus the old bare `create_workspace` HTTP risk reachable; the candidate was restored for the manifest-backed retry.
  - Attempt 2 (2026-08-24): Independent Checker PASS; exact Verify passed `8 passed, 30 deselected`; related adapter/HTTP/project-path regression passed `70 passed`; application project-service/request-contract regression passed `16 passed`; target Ruff and `git diff --check` passed, and `./lint-loop.sh` passed before this lifecycle move (`open=4 done=189 blocked=0`). CodeGraph/source inspection and independent AST/black-box probes confirmed strict `project`/`manifest`/`source` creation, opaque response identity with no runtime-root disclosure, persisted manifest/source binding, client identity mutation rejection, manifest tamper rejection, bare Markdown/`web-upload`/raw workspace rejection, absolute/URL/traversal/non-Markdown/source-mismatch failure cleanup, and retained low-level artifact/live-preview coverage; no fallback or silent downgrade was introduced.
...
- [REG-008] Verify the installed distribution through a manifest-backed project entry
  ...
  - Verification-surface change: `no`
  - Attempts: 1
  - Attempt 1 (2026-08-24): Independent Checker PASS; exact Verify passed 1/1 offline, the installed verifier reported `hermetic=true` and a valid DOCX ZIP package of 31,003 bytes, manifest-backed inspect/validate passed, and bare Markdown remained rejected with `TF-PROJECT-ENTRY-REQUIRED`; target Ruff, `git diff --check`, and LOOP-LINT passed.
...
- [V2-535A] Migrate remaining RenderNode test consumers
  ...
  - Verification-surface change: `no`
  - Attempts: 1
  - Attempt 1 (2026-08-23): Checker PASS; exact Verify passed 12/12; independent AST/import audit confirmed no `RenderNode` or `to_render_node()` in either named test, retained heading/section/bibliography `kind`/`payload` and sequence `field_code` assertions, and explicit Review unknown-instruction and inline-unknown TypeError boundaries; target Ruff, `git diff --check`, and `./lint-loop.sh` passed (`open=3 done=168 blocked=0` before this lifecycle move); candidate product diff was exactly the two named test files, no compatibility fixture or weakened assertion was introduced, all pre-existing `openspec/**`, `tests/test_lo_finalizer.py`, `openspec/.specnav/overrides/`, and `template-v2-build-pipeline-p1/development/` paths were preserved, no push.
...
- [V2-534B] Remove preview RenderNode dispatch
  ...
  - Verification-surface change: `yes`
  - Attempts: 1
  - Attempt 1 (2026-08-23): Checker PASS; exact Verify passed 12/12; the independent unknown-object assertion returned kind/status `unsupported` and content `{"type": "unsupported", "originalKind": "UnknownInstruction"}`, with no `RenderNode` attribute on the preview module; independent AST/import audit confirmed no `RenderNode` import/name and an explicit unsupported terminal boundary; no fallback, compatibility alias, or silent-acceptance path was found. Target Ruff, `git diff --check`, and `./lint-loop.sh` passed (`open=2 done=166 blocked=0` before this lifecycle move); candidate product diff before the lifecycle update was exactly `src/thesis_forge/presentation/preview.py` (7 deletions), all pre-existing `openspec/**`, `tests/test_lo_finalizer.py`, and `openspec/.specnav/overrides` paths were preserved, no push.
...
- [V2-539A] Add the typed Review Markdown serializer
  ...
  - Closed (2026-08-23): superseded by Done `V2-539A-R1`, `V2-539B` and `V2-539C`; the fresh Review serializer, source-map and CLI export chain was independently verified green, with no product code edited during this ledger closure.
...
- 2026-08-23 - Historical Blocked ledger closed; `V2-539A` moved verbatim into `## Blocked archive` after fresh replacement `V2-539A-R1` and dependent `V2-539B/C` completed, preserving all failed-attempt evidence and leaving no active Blocked items; no product code edited.
...
- 2026-08-24 - REG-015 split into ordered children REG-015A through REG-015D after CodeGraph and current source inspection showed that the failed real HTTP acceptance requires separate manifest-backed Web workspace/API, typed Web project transport, browser project-entry and real acceptance seams; each child names at most three files with its own Verify command, the original REG-015 Behavior/Acceptance and Attempt 1 evidence were preserved, no product code was edited, and the next queue is REG-015A.
- 2026-08-24 - REG-015A Checker PASS Attempt 2; exact Web adapter Verify passed 8/8 (30 deselected), related adapter/HTTP/project-path regression passed 70/70, application project-service/request-contract regression passed 16/16, target Ruff, `git diff --check`, LOOP-LINT, CodeGraph/source audit, and independent AST/black-box boundary probes passed; manifest-backed creation, opaque identity, persisted workspace/manifest dispatch binding, strict path/source validation, failed cleanup, bare/legacy rejection, low-level artifact/live-preview compatibility, and no-fallback scope were verified.
- 2026-08-24 - REG-015B Checker PASS Attempt 1; exact focused Web project transport passed `52/52`, package-script coverage passed `19` files/`231/231`, frontend typecheck/lint, `git diff --check` and LOOP-LINT passed; CodeGraph/current-source audit and independent black-box probes confirmed exact manifest-backed input, one `{project, manifest, source}` workspace POST, strict protocol/identity/source response rejection, shared Tauri/Web `desktop`/`web-workspace` parser, project `web-upload` rejection, retained source-only `web-upload`, and no fallback or `dto.ts`/scope breach.
- 2026-08-24 - REG-015C Checker FAIL Attempt 1; exact Verify failed with `229/231` tests passed and two stale browser-project assertions, standalone typecheck passed, and `git diff --check` passed; independent audit confirmed the existing ProductBar YAML-only and WorkbenchApp Web source-only tests conflict with the manifest-backed project contract, while the candidate also used frontend YAML scalar parsing without directory selection. The three candidate product files were restored, REG-015C was re-sliced into ordered C1/C2/C3 children with authorized test migration and a new WorkbenchShell seam test, REG-015D now depends on C3, no commit or push.
- 2026-08-24 - REG-015C1 Checker FAIL Attempt 1; exact Verify passed `19` files/`231/231` tests, frontend typecheck/lint, `git diff --check`, and `./lint-loop.sh` passed; independent audit found the candidate `File | File[]` callback was an unsafe old/new dual protocol for unchanged `File`-only WorkbenchShell/WorkbenchApp callers and still routed multi-file selection toward source-only `openFile`/`openSource` instead of typed manifest-backed `openProject`; the two candidate product/test files were restored, REG-015C1 was re-sliced into ordered C1A/C1B/C1C preparation children so each consumer seam is migrated without a dual protocol, unrelated dirty/untracked paths were preserved, no commit or push.
- 2026-08-24 - REG-015C-ATOM Checker PASS Attempt 1; exact Verify passed backend `9/9` and frontend `72/72`, frontend typecheck, lint, target Ruff, `git diff --check`, and LOOP-LINT passed; independent scope and contract audit confirmed exact `{manifest, source}` creation, manifest/source binding and manifest-derived identity, retained ProductBar `onChooseSource` routing, single `File[]` browser selection, no WorkbenchApp `openSource` or YAML parsing, unchanged Tauri picker and operation identity payloads, and split client-project/duplicate-metadata negative assertions; no finding, REG-015C-ATOM moved from Open to Done, REG-015D and historical records were unchanged, unrelated dirty paths were preserved and unstaged, one local commit, no push.
- 2026-08-24 - V2-618 final Checker PASS; `./stop-check.sh` exited `0`: LOOP-LINT reported `open=0 done=196 blocked=0`, Goal verifier passed, `make verify` passed (`1281` Python tests, `233` frontend tests, frontend typecheck/lint/build, real HTTP Playwright, Tauri tests, sidecar offline smoke, distribution verification and strict OpenSpec validation), and `git diff --check` passed; all Open and Blocked sections are empty, Status is `done`, no push or release action was performed.
